...
const inputClass = 'h-12 rounded-2xl border border-[var(--editable-border)] bg-[#0a1a1a]/85 px-4 text-base font-bold text-current outline-none transition placeholder:text-current/35 focus:border-current focus:bg-[#0a1a1a]'
const buttonClass = 'inline-flex h-12 items-center justify-center rounded-2xl bg-[var(--editable-page-text,#040D12)] px-6 text-sm font-black uppercase tracking-[0.18em] text-[var(--editable-page-bg,#ffffff)] shadow-sm transition hover:-translate-y-0.5 disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-2xl px-4 py-3 text-sm font-bold ${status === 'success' ? 'bg-[#5C8374]/15 text-[#93B1A6]' : 'bg-red-900/20 text-red-400'}`}>{message}</p> : null}
