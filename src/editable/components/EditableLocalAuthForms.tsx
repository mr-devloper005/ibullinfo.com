...
const inputClass = 'h-12 rounded-[1.2rem] border border-black/8 bg-[#fbf9f4] px-4 text-base font-medium text-current outline-none transition placeholder:text-black/35 focus:border-[#005c6b] focus:bg-white'
const buttonClass = 'inline-flex h-12 items-center justify-center rounded-[1.2rem] bg-[#005c6b] px-6 text-sm font-black uppercase tracking-[0.22em] text-white transition hover:-translate-y-0.5 disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!user || user.password !== password) {
      setStatus('error')
      setMessage('No local account found with these details. Create an account first, then log in.')
      return
    }
    saveSession(user)
    setStatus('success')
