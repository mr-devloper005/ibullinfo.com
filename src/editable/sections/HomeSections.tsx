...
import { ArticleListCard, CompactIndexCard, EditorialFeatureCard, HorizontalCard, RailPostCard, getEditableCategory, getEditableExcerpt, getEditablePostImage, postHref } from '@/editable/cards/PostCards'
...
export function EditableHomeHero({ primaryTask, primaryRoute, posts, timeSections }: HomeSectionProps) {
  ...
  return (
    <section className="relative overflow-hidden bg-white">
      ...
      <div className="slot4-skyline relative overflow-hidden border-b border-black/5 bg-[linear-gradient(180deg,rgba(255,255,255,0.98)_0%,rgba(255,255,255,0.92)_74%,rgba(255,255,255,0.98)_100%)]">
        ...
        <div className="mx-auto max-w-[var(--editable-container,1440px)] px-4 pb-7 pt-8 sm:px-6 lg:px-8 lg:pb-10 lg:pt-10">
          <div className="mx-auto grid max-w-[1180px] items-center gap-8 lg:grid-cols-[1.05fr_0.95fr]">
            <div className="slot4-fade-up relative z-10">
              ...
              <h1 className="mt-5 max-w-[620px] text-balance text-[clamp(2.6rem,7vw,5.7rem)] font-black leading-[0.95] tracking-[-0.07em] text-[#101828]">
                Find what you need, post what you want, and keep moving.
              </h1>
              <p className="mt-5 max-w-[590px] text-[clamp(1rem,1.6vw,1.08rem)] leading-8 text-black/62">
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                <Link href={primaryRoute} className="inline-flex items-center gap-2 rounded-full bg-[#005c6b] px-6 py-3.5 text-sm font-black text-white shadow-[0_16px_28px_rgba(0,92,107,0.18)] transition hover:-translate-y-0.5 hover:bg-[#004c58]">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                {['Fast discovery', 'Easy publishing', 'All categories in one place'].map((item) => (
                  <span key={item} className="rounded-full border border-black/8 bg-white px-4 py-2 text-xs font-medium text-black/64 shadow-sm">
                    {item}
                  </span>
                ))}
              </div>
            ...
            <div className="slot4-fade-up relative z-10 lg:justify-self-end">
              <div className="slot4-card relative overflow-hidden rounded-[28px] p-4 sm:p-5">
                <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(245,205,58,0.14),transparent_38%),linear-gradient(180deg,rgba(255,255,255,0.98),rgba(255,255,255,0.94))]" />
                <div className="relative grid gap-4">
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      <p className="text-[11px] font-black uppercase tracking-[0.34em] text-black/45">Spotlight</p>
                      <h2 className="mt-2 text-2xl font-black tracking-[-0.05em] text-[#101828]">A clean place to explore, compare, and publish.</h2>
                    </div>
                    <span className="rounded-full bg-[#f5cd3a] px-3 py-1 text-[11px] font-black uppercase tracking-[0.22em] text-black">
                      Live
                    </span>
                  </div>
                  <div className="grid gap-3 sm:grid-cols-[1.15fr_0.85fr]">
                    <article className="overflow-hidden rounded-[24px] bg-[#0f172a] text-white shadow-[0_16px_35px_rgba(15,23,42,0.16)]">
                    ...
                    <div className="grid gap-3">
                      {supportingPosts.map((post, index) => (
                        <Link key={post.slug || post.id || post.title || index} href={postHref(primaryTask, post, primaryRoute)} className="group rounded-[22px] border border-black/8 bg-white/88 p-4 shadow-[0_12px_28px_rgba(15,23,42,0.06)] transition hover:-translate-y-0.5 hover:shadow-[0_18px_34px_rgba(15,23,42,0.1)]">
                          <p className="text-[10px] font-black uppercase tracking-[0.28em] text-black/42">{getEditableCategory(post) || taskLabel(primaryTask)}</p>
                          <h4 className="mt-2 line-clamp-2 text-[15px] font-black leading-snug text-black">{post.title}</h4>
                          <p className="mt-2 line-clamp-2 text-sm leading-6 text-black/60">{safeSummary(post, 84) || 'Compact space for a second look and quick details.'}</p>
                        </Link>
                      ))}
                    </div>
                  </div>
      ...
      <div className="mx-auto max-w-[var(--editable-container,1440px)] px-4 pb-8 pt-2 sm:px-6 lg:px-8">
        <div className="slot4-card rounded-[28px] p-4 sm:p-5 lg:p-6">
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              <p className="text-[11px] font-black uppercase tracking-[0.34em] text-[#005c6b]">Categories</p>
              <h2 className="mt-2 text-2xl font-black tracking-[-0.05em] text-[#101828] sm:text-3xl">Quick jumps to all categories.</h2>
            </div>
            <div className="flex items-center gap-2">
              <button type="button" onClick={() => scrollCategoryRail(-1)} className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-black/10 bg-white text-black/70 shadow-sm transition hover:bg-black/[0.03]" aria-label="Previous categories">
                <ChevronLeft className="h-5 w-5" />
              </button>
              <button type="button" onClick={() => scrollCategoryRail(1)} className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-black/10 bg-white text-black/70 shadow-sm transition hover:bg-black/[0.03]" aria-label="Next categories">
          ...
          <div ref={categoryRailRef} className="slot4-carousel mt-6 flex gap-3 overflow-x-auto pb-2">
            {categoryItems.map(({ category, icon: Icon, count }) => (
              <div
                key={category.slug}
                className="group flex min-w-[150px] shrink-0 flex-col rounded-[22px] border border-black/8 bg-white px-4 py-4 text-left shadow-[0_10px_24px_rgba(15,23,42,0.05)] transition hover:-translate-y-0.5 hover:border-[#005c6b]/20 hover:shadow-[0_16px_32px_rgba(15,23,42,0.08)]"
              >
                <span className="flex h-10 w-10 items-center justify-center rounded-full bg-[#f7fbfc] text-[#005c6b] transition group-hover:scale-105">
                  <Icon className="h-5 w-5" />
                </span>
                <span className="mt-4 text-[15px] font-black leading-snug text-black/80">{category.name}</span>
              </div>
            ))}
          </div>
        </div>
...
export function EditableStoryRail({ primaryTask, primaryRoute, posts, timeSections }: HomeSectionProps) {
  ...
  return (
    <section className="border-y border-black/5 bg-[#faf8f4]">
      <div className="mx-auto max-w-[var(--editable-container,1440px)] px-4 py-8 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-[11px] font-black uppercase tracking-[0.24em] text-[#005c6b]">Categories</p>
          ...
          <div className="hidden items-center gap-2 lg:flex">
            <button type="button" onClick={() => scrollByDirection(-1)} className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-black/8 bg-white text-black/70 transition hover:bg-black/[0.03]" aria-label="Scroll categories left">
              <ChevronLeft className="h-5 w-5" />
            </button>
            <button type="button" onClick={() => scrollByDirection(1)} className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-black/8 bg-white text-black/70 transition hover:bg-black/[0.03]" aria-label="Scroll categories right">
              <ChevronRight className="h-5 w-5" />
            </button>
          </div>
        </div>
        ...
        <div ref={ref} className="slot4-carousel mt-6 flex gap-5 overflow-x-auto pb-2">
          {taskCards.map(({ task, count, icon: Icon }) => (
            <Link
              key={task.key}
              href={task.route}
              className="group flex min-w-[170px] shrink-0 flex-col items-center rounded-[1.8rem] border border-black/8 bg-white px-5 py-5 text-center shadow-[0_12px_32px_rgba(15,23,42,0.05)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_18px_48px_rgba(15,23,42,0.1)] sm:min-w-[190px]"
            >
              <span className="flex h-14 w-14 items-center justify-center rounded-[1.4rem] bg-[#f7fbfc] text-[#005c6b]">
                <Icon className="h-7 w-7" />
              </span>
              <span className="mt-4 text-[15px] font-semibold leading-tight tracking-[-0.03em] text-[var(--slot4-page-text)]">
                {task.label}
              </span>
              <span className="mt-2 text-sm font-bold text-[#005c6b]">({count})</span>
            </Link>
          ))}
          {!taskCards.length ? (
            <div className="rounded-[1.4rem] border border-dashed border-black/10 bg-white px-5 py-4 text-sm text-black/55">
    ...
    <section className="bg-white">
      <div className="mx-auto max-w-[var(--editable-container,1440px)] px-4 py-10 sm:px-6 lg:px-8">
        <div className="flex items-end justify-between gap-4">
          <div>
            <p className="text-[11px] font-black uppercase tracking-[0.24em] text-[#005c6b]">Featured Business</p>
          ...
          <div className="hidden items-center gap-2 lg:flex">
            <button type="button" onClick={() => scrollByDirection(-1)} className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-black/8 bg-[#f7f5ef] text-black/70 transition hover:bg-black/[0.03]" aria-label="Previous featured card">
              <ChevronLeft className="h-5 w-5" />
            </button>
            <button type="button" onClick={() => scrollByDirection(1)} className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-black/8 bg-[#f7f5ef] text-black/70 transition hover:bg-black/[0.03]" aria-label="Next featured card">
              <ChevronRight className="h-5 w-5" />
            </button>
          </div>
        </div>
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <div className="rounded-[1.6rem] border border-black/8 bg-[#f9f7f1] p-5">
            <p className="text-[11px] font-black uppercase tracking-[0.24em] text-[#005c6b]">Why it works</p>
            <p className="mt-3 text-sm leading-7 text-black/65">
              {pagesContent.home.hero.featureCardDescription}
            </p>
          </div>
          {timeSections.slice(0, 3).map((section) => (
            <Link key={section.key} href={section.href} className="rounded-[1.6rem] border border-black/8 bg-white p-5 transition hover:-translate-y-1 hover:shadow-[0_18px_48px_rgba(15,23,42,0.08)]">
              <p className="text-[11px] font-black uppercase tracking-[0.24em] text-[#005c6b]">{section.eyebrow}</p>
              <h3 className="mt-3 text-xl font-black tracking-[-0.04em] text-[var(--slot4-page-text)]">{section.title}</h3>
              <p className="mt-3 text-sm leading-7 text-black/60">{section.description}</p>
            </Link>
          ))}
        </div>
      </div>
    ...
    <section className="bg-[#faf8f4]">
      <div className="mx-auto max-w-[var(--editable-container,1440px)] px-4 py-10 sm:px-6 lg:px-8">
        <div className="mx-auto mb-8 max-w-5xl rounded-[1.8rem] border border-black/8 bg-[#f7df58] p-6 shadow-[0_18px_60px_rgba(15,23,42,0.08)]">
          <div className="grid gap-4 lg:grid-cols-[1fr_auto] lg:items-center">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-3xl font-black leading-none tracking-[-0.06em] text-black sm:text-4xl">Post, promote, and browse faster.</p>
                <p className="mt-2 text-lg leading-7 text-black/85">A cleaner lane for local listings, offers, and useful pages.</p>
              </div>
            </div>
        ...
        <div className="grid gap-8 xl:grid-cols-[1.05fr_0.95fr]">
          <div className="rounded-[2rem] border border-black/8 bg-white p-5 shadow-[0_18px_60px_rgba(15,23,42,0.06)] sm:p-6">
            <div className="flex items-center justify-between gap-4">
              <div>
              ...
              <Link href={primaryRoute} className="hidden text-sm font-bold text-[#005c6b] hover:underline sm:inline-flex">
                ...
                {sidebarEntries.flatMap((section) => section.posts.slice(0, 2)).map((post, index) => {
                  ...
                  return (
                    <div key={post.id || post.slug || index} className="rounded-[1.2rem] border border-black/6 bg-[#fbf9f4] p-4">
                      ...
                      <p className="mt-2 text-sm leading-6 text-black/60">{location}</p>
                    </div>
                  )
                })}
                {!sidebarEntries.length ? (
                  <div className="rounded-[1.2rem] border border-dashed border-black/10 bg-[#fbf9f4] p-4 text-sm text-black/55">
                ...
                {articlePosts.map((post, index) => (
                  <div key={post.id || post.slug || index} className="rounded-[1.2rem] border border-black/6 bg-[#fbf9f4] p-4">
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <span className="rounded-full bg-[#005c6b] px-3 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-white">
                        {index + 1}
                      </span>
                    </div>
                    <p className="mt-3 line-clamp-3 text-sm leading-6 text-black/62">{safeSummary(post, 130)}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="mt-10 grid gap-8 xl:grid-cols-[1fr_420px]">
          ...
          <div className="space-y-5">
            <div className="rounded-[2rem] border border-black/8 bg-white p-5 shadow-[0_18px_60px_rgba(15,23,42,0.06)]">
              <p className="text-[11px] font-black uppercase tracking-[0.24em] text-[#005c6b]">Top Searches</p>
              <div className="mt-4 flex flex-wrap gap-2">
                {['Rent a room', 'Mobile service', 'Wedding photographer', 'Used furniture', 'Project work', 'Local tutor', 'Office space', 'Marketing help'].map((item) => (
                  <Link key={item} href="/search" className="rounded-full border border-black/8 bg-[#faf8f4] px-4 py-2 text-sm font-medium text-black/70 transition hover:-translate-y-0.5 hover:bg-black/[0.03]">
                    {item}
                  </Link>
                ))}
              </div>
            ...
            <div className="rounded-[2rem] border border-black/8 bg-[#0f5d68] p-6 text-white shadow-[0_18px_60px_rgba(15,23,42,0.12)]">
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                <Link href="/contact" className="rounded-full bg-[#f3cc37] px-5 py-3 text-sm font-black text-black transition hover:-translate-y-0.5">
                  Contact us
                </Link>
  ...
  return (
    <section className="bg-white">
      <div className="mx-auto max-w-[var(--editable-container,1440px)] px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
        <div className="rounded-[2rem] border border-black/8 bg-[#f7f5ef] p-6 shadow-[0_18px_60px_rgba(15,23,42,0.05)]">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="max-w-3xl">
              <p className="text-[11px] font-black uppercase tracking-[0.24em] text-[#005c6b]">Ready to publish?</p>
            ...
            <div className="flex flex-wrap gap-3">
              <Link href={pagesContent.home.cta.primaryCta.href} className="inline-flex items-center gap-2 rounded-full bg-[#005c6b] px-5 py-3 text-sm font-bold text-white transition hover:-translate-y-0.5">
                {pagesContent.home.cta.primaryCta.label}
                <ArrowRight className="h-4 w-4" />
              </Link>
              <Link href={pagesContent.home.cta.secondaryCta.href} className="inline-flex items-center gap-2 rounded-full border border-black/8 bg-white px-5 py-3 text-sm font-bold text-black transition hover:-translate-y-0.5 hover:bg-black/[0.03]">
